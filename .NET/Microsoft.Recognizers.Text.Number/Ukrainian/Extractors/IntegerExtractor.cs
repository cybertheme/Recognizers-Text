    ...
    public class IntegerExtractor : BaseNumberExtractor
    {
        ...
        public IntegerExtractor(string placeholder = @"\D|\b")
        {
            var regexes = new Dictionary<Regex, TypeTag>
            {
                {
                    new Regex(
                        $@"(((?<!\d+\s*)-\s*)|(?<=\b))\d+(?!(\.\d+[a-zA-Z]))(?={placeholder})",
                        RegexOptions.IgnoreCase | RegexOptions.Singleline),
                    RegexTagGenerator.GenerateRegexTag(Constants.INTEGER_PREFIX, Constants.NUMBER_SUFFIX)
                },
                {
                    new Regex(
                        @"(((?<!\d+\s*)-\s*)|(?<=\b))\d+\s*(тис|млн|млрд|трлн)(?=\b)", RegexOptions.Singleline),
                    RegexTagGenerator.GenerateRegexTag(Constants.INTEGER_PREFIX, Constants.NUMBER_SUFFIX)
                },
                {
                    new Regex(
                        @"(((?<!\d+\s*)-\s*)|(?<=\b))\d{1,3}(,\d{3})+" + $@"(?={placeholder})",
                        RegexOptions.IgnoreCase | RegexOptions.Singleline),
                    RegexTagGenerator.GenerateRegexTag(Constants.INTEGER_PREFIX, Constants.NUMBER_SUFFIX)
                },
                {
                    new Regex(
                        $@"(?<=\b)\d+\s+{RoundNumberIntegerRegex}(?=\b)",
                        RegexOptions.IgnoreCase | RegexOptions.Singleline),
                    RegexTagGenerator.GenerateRegexTag(Constants.INTEGER_PREFIX, Constants.NUMBER_SUFFIX)
                },
                {
        ...
        public static string BelowHundredsRegex =>
            $@"((({TensNumberIntegerRegex})(\s+)?({ZeroToNineIntegerRegex})?)|({ZeroToNineIntegerRegex})|({TenToNineteenIntegerRegex}))";

        public static string BelowThousandsRegex =>
            $@"((({HundredsNumberIntegerRegex})?(\s+)?({BelowHundredsRegex}))|({HundredsNumberIntegerRegex})|({BelowHundredsRegex}))";

        public static string BelowMlnRegex =>
            $@"((({BelowThousandsRegex})?(\s+)?({RoundNumberIntegerRegex})(\s+)?({BelowThousandsRegex})?)|({BelowThousandsRegex}))";

        public static string BelowBlnRegex =>
            $@"((({BelowThousandsRegex})?(\s+)?({RoundNumberIntegerRegex})(\s+)?({BelowMlnRegex})?)|({BelowMlnRegex}))";

        public static string BelowTrlnRegex =>
            $@"((({BelowThousandsRegex})?(\s+)?({RoundNumberIntegerRegex})(\s+)?({BelowBlnRegex})?)|({BelowBlnRegex}))";

        public static string AllIntRegex =>
            $@"((({BelowThousandsRegex})?(\s+)?({RoundNumberIntegerRegex})(\s+)?({BelowTrlnRegex})?)|({BelowTrlnRegex}))";
